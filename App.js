...
export default function App(){
  ...
  async function saveEditMealAI(){
    ...
    const prompt=`Analiza este alimento/receta y devuelve SOLO JSON sin texto ni marcas:
${text}
Formato: {"name":"nombre (máx 40 chars)","desc":"ingredientes (máx 60 chars)","emoji":"emoji","kcal":número,"protein":gramos,"carbs":gramos,"fat":gramos,"portion":"porción"}`;
    try{
        ...
        <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={()=>{setShowAdd(false);resetAdd();setEditingEntryIdx(null);}}>
          <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={{width:'100%'}}>
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
              <View style={s.sheet} onStartShouldSetResponder={()=>true} onTouchEnd={e=>e.stopPropagation()}>
